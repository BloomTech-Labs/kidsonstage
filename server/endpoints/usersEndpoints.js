...
usersRouter.put('/update', function(req, res) {
	const { id, email, phoneNumber, byEmail, byPhone, password, newPassword } = req.body;
	
	// If both password and new password left blank - means not changing password
	if ((password === undefined && newPassword === undefined) || (password && newPassword === undefined)) {
		db('users')
			.where('id', id)
				...
				bcrypt.compare(password, result[0].password, function(err2, res2) {
					...
					if (res2 === true) {
						...
						// Store hash in your password DB.
							...
							db('users')
								.where('id', id)
								.update({ email, phoneNumber, byEmail, byPhone, password: hash })
								.then(function(record) {
									if (record) {
										res.status(200).json(record);
									} else {
